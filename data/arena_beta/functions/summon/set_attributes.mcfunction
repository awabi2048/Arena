...
# 固有のデータを設定
execute as @e[tag=Arena.Summoned,type=creeper] if score $Temp.Wave Arena matches ..5 store result entity @s ExplosionRadius byte 0.66 run scoreboard players get $Temp.Wave Arena
execute as @e[tag=Arena.Summoned,type=creeper] unless score $Temp.Wave Arena matches ..5 store result entity @s ExplosionRadius byte 0.1 run scoreboard players get $Temp.Wave Arena
