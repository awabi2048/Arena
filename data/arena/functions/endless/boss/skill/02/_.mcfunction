# シュルカーの弾をプレイヤーに向けて飛ばす (ホーミング)
# 弾召喚コマンドはarena:Boss/functions/endless/boss/tickにある
...
execute at @e[tag=Arena.Boss.ShulkerBullet] as @a[distance=..1] run effect give @s instant_damage 2 1 true
execute at @e[tag=Arena.Boss.ShulkerBullet] as @a[distance=..1] run effect give @s levitation 3 1 true
execute at @e[tag=Arena.Boss.ShulkerBullet] as @a[distance=..1] run playsound entity.generic.explode master @a ~ ~ ~ 0.5 2
execute as @e[tag=Arena.Boss.ShulkerBullet] at @s if entity @a[distance=..1] run kill @s
