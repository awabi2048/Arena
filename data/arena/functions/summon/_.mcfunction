# エンドレス
        ## 各ボスの召喚判定
        execute store result score $Temp.BossSummon Arena run data get entity @e[tag=Arena.Core,sort=nearest,limit=1] data.Wave -1
        execute store result score $Temp.Wave Arena run data get entity @e[tag=Arena.Core,sort=nearest,limit=1] data.Wave
        execute if data entity @e[tag=Arena.Core,sort=nearest,limit=1] {data:{Endless:1b}} run scoreboard players operation $Temp.Wave Arena *= #-1 Arena
        ...
        execute unless data entity @e[tag=Arena.Core,sort=nearest,limit=1] {data:{Endless:1b}} run scoreboard players set $Temp.BossSummon Arena 0
...
# 通常アリーナ
    ...
    execute store result score $Temp.Wave Arena run data get entity @e[tag=Arena.Core,sort=nearest,limit=1] data.Wave
    execute if data entity @e[tag=Arena.Core,sort=nearest,limit=1] {data:{Endless:1b}} run function arena:endless/status_multiple
