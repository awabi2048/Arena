...
scoreboard players operation @a[tag=Arena.Player,scores={Arena=0}] Arena = @e[tag=Arena.Entrance,sort=nearest,limit=1] Arena
    ...
    # 裏エンドレス用に人数カウント
    execute store result storage arena:temp PlayerCount byte 1 if entity @a[distance=..3.5]
    ...
    execute as @e[tag=Arena.Core] if score @s Arena = @e[tag=Arena.Entrance,sort=nearest,limit=1] Arena at @s if data entity @s {data:{Arena:{StageType:Endless}}} if data storage arena:temp {PlayerCount:1b} if entity @p[advancements={arena:display/endless/solo_clear=true}] run scoreboard players enable @p[tag=Arena.Player] Arena.PlayHiddenStage
    execute as @e[tag=Arena.Core] if score @s Arena = @e[tag=Arena.Entrance,sort=nearest,limit=1] Arena at @s if data entity @s {data:{Arena:{StageType:Endless}}} if data storage arena:temp {PlayerCount:1b} if entity @p[advancements={arena:display/endless/solo_clear=true}] run tellraw @p[tag=Arena.Player] [{"text":"＊エンドレスアリーナ 隠しステージに進みますか？ ","color":"#dc143c","bold": true},{"text":"[クリックして続行]","color":"aqua","clickEvent": {"action": "run_command","value": "/trigger Arena.PlayHiddenStage set 1"}}]
