...
data modify storage arena:temp Enterable set value true
    ...
    execute unless score $PlayerCount Arena.Temp matches ..4 at @e[tag=Arena.Entrance,sort=nearest,limit=1] run tellraw @a[distance=..3.5] [{"text":"[","color": "white"},{"text":"Arena","color": "red"},{"text":"] ","color": "white"},{"text":"プレイヤーが多すぎます！ 枠内のプレイヤーを4人以下にした状態で再度ボタンを押してください。"}]
