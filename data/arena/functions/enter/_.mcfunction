...
scoreboard players set $Temp.Enterable Arena 1
    ...
    # 同じArenaスコアを持つ人がいない(このアリーナに誰もいない)場合にエラー
...
execute if score $Temp.Enterable Arena matches 1 at @e[tag=Arena.Entrance,sort=nearest,limit=1] run function arena:enter/success
