...
execute if score $Temp.Wave Arena matches -1..1 at @e[tag=Arena.Core,sort=nearest,limit=1] store result entity @e[tag=Arena.Core,sort=nearest,limit=1] data.Arena.PlayerCount int 1 if entity @a[tag=Arena.Player,distance=..32]

# エンドレス隠しステージ入場の処理
execute as @a[tag=Arena.Player] if score @s Arena.PlayHiddenStage matches 1 run playsound block.beacon.power_select master @s ~ ~ ~ 5 0.84
