...
# 開始時の処理: 看板削除, タイマー用エンティティの削除, 人数を保存
...
execute if score $Temp.Wave Arena matches 1 at @e[tag=Arena.Core,sort=nearest,limit=1] store result entity @e[tag=Arena.Core,sort=nearest,limit=1] data.Arena.PlayerCount byte 1 if entity @a[tag=Arena.Player,distance=..32]
